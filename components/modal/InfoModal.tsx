...
const Modal = ({ children, show = false, closeModal, titleHtml }: ModalParams) => {
  ...
  return (
    <div className={`w-screen h-screen bg-modal fixed top-0 left-0 ${showHideClassName}`}>
      <div
        ref={selectRef}
        className='bg-white rounded-12t fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 boder border-solid border-secondary_fill w-11/12 mx-auto lg:w-1/3'>
        <div className='flex justify-between bg-secondary_fill p-16t rounded-t-12t'>
          <h3 className='text-black_login font-medium text-18t'>{titleHtml}</h3>
          <Cross onClick={() => closeModal()} className='fill-third cursor-pointer' />
        </div>
        <div className='p-16t'>
          {children}
        </div>
