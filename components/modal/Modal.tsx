...
const Modal =  ({children, show=false, closeModal, customClass}: IModal) => {
  const showHideClassName = show ? "block" : "hidden";
  return (
    <div className={`w-screen h-screen bg-modal fixed top-0 left-0 ${showHideClassName}`}>
      <div
        className={`bg-white rounded-12t fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 boder border-solid border-secondary_fill p-24t w-11/12 mx-auto ${customClass}`}>
