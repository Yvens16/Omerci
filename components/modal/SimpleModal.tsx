...
const SimpleModal = ({ customClass, children, show = false, closeModal, titleHtml,  }: IInfoModal) => {
  ...
  return (
    <div className={`w-screen h-screen bg-modal fixed top-0 left-0 ${showHideClassName}`}>
      ...
      <div
        ref={selectRef}
        className={`bg-white rounded-12t fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 boder border-solid border-secondary_fill w-11/12 mx-auto md:max-w-1/3 lg:max-w-[541px] ${customClass}`}>
        <div className='p-16t'>
          {children}
        </div>
      </div>
