...
const Cards = ({ cards, sectionName, activeIndex, toggleOption, mobileOption, desktopOption, sent }: TCards) => {
  const whichCards = (cards: {
    ...
    if (cards.length > 0) {
      return cards.filter(card => card.isSent === sent).map((card, id) => (
        <div className={`${id === activeIndex ? "z-10" : ""} relative carte card_box_shadow lg:mb-16t mr-16t rounded-12t bg-white border border-solid min-w-[235px] drop-shadow-[0_5px_12px_rgba(0,0,0,0.05)] border-secondary_fill`} key={id}>
          ...
          <div className='flex flex-col items-center p-24t'>
            <div className=' mb-8t relative overflow-hidden	border border-solid border-white  w-[62px] h-[62px] rounded-16t flex justify-center items-center text-white text-[20px] font-semibold'>
              {card.photoUrl.length
                ? <Image src={card.photoUrl} alt={`Photo de la carte ${card.title}`} layout='fill' objectFit='cover'/>
                : <Image src={avatars[Math.floor(Math.random() * avatars.length)]} alt={`Photo de la carte ${card.title}`} layout='fill' objectFit='cover'/>}
            </div>
    ...
    if (sent === false) {
      return <EmptyCard imgLink='/emptyCard.svg' text="Vous n'avez pas encore de carte crée" addNewCard={function (): void | Promise<void> {
